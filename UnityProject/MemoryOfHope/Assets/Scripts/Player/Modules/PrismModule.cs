...
public class PrismModule : Module
{
    ...
    public override void Execute()
    {
       ...
       // reprendre le code du mouvement bouclier en enfant
       // bouclier il est de la vie
       // bouclier peut se casser
       // il réaparait au bout d'un temps
       // si tu recois un laser alors tu le donnes la direction 
       //
       isPerformed = true;
       if (!isActivate)
       {  
       ...
       else
       {
           if(!_shield.isDead)
              ...
              Vector2 angleFoward = new Vector2(transform.forward.x,
                      transform.forward.z);
                  Vector2 rotationVector = Vector3.RotateTowards(angleFoward, _joystickDirection, rotationSpeed, 00f);
